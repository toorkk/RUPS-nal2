export function spawnNandForCurrentChallenge(scene) {
  ...
  const panelWidth = 150; // prilagodi, če imaš drugo širino panela

  // če že obstaja, jo odstranimo
  if (scene.nandContainer) {
    scene.nandContainer.destroy();
    scene.nandContainer = null;
  }

  scene.nandState = { a: 0, b: 0 };
  ...
  const title = scene.add.text(0, 0, 'NAND', {
    fontSize: '14px',
    color: '#000000',
    fontStyle: 'bold'
  }).setOrigin(0.5);
  ...
  circleA.on('pointerdown', () => {
    scene.nandState.a = scene.nandState.a ? 0 : 1;
    setCircleState();
  });

  circleB.on('pointerdown', () => {
    scene.nandState.b = scene.nandState.b ? 0 : 1;
    setCircleState();
  });
